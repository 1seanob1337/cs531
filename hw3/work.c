...
void inorder(struct address_t *, void (struct address_t *, char[], FILE *), FILE *);
void print_iden(struct address_t *, char[], FILE *);
void print_file_format(struct address_t *, char[], FILE *);
int compare_alias(void *, struct address_t *);
struct address_t *create_node(char *, int *);
struct address_t *insert(struct address_t *, struct address_t *);
char *octet_to_string(int *octet);
int validate_addr(int *, int, struct address_t *);
int look_up_address(struct address_t *);
struct address_t *deleteNode(struct address_t *, char *);
int display_alias_for_location(struct address_t *);

void my_flush(FILE *);
int compare_half(void *key, struct address_t *node);
int save_file(struct address_t *);
char *node_string(struct address_t *);
int node_max_depth(struct address_t *, struct address_t *);
int node_max_height(struct address_t *, struct address_t *);
int max(int, int);
...
{
    struct address_t *new_entry = (struct address_t *)malloc(sizeof(struct address_t));
    if (new_entry == NULL)
    {
...
struct address_t *read_file(struct address_t *head)
{
    ...
    while (fgets(line, 50, fp) != NULL)
    {
        ...
        if (node == NULL)
            return NULL;
        // printf("before insert of %s\t head=%s\n",node->alias,head->alias);
...
void inorder(struct address_t *head, void (*print_function)(struct address_t *, char message[], FILE *fp),FILE *fp)
{
    if (head != NULL)
    {
...
void *foldWhile(void *key,
                struct address_t *head,
                int (*compare)(void *, struct address_t *),
                void *(*f)(struct address_t *, void *acc),
                void *acc)
{
    if (head)
    {
...
struct address_t *searchInOrder(void *key, struct address_t *head, int (*compare)(void *, struct address_t *))
{
...
struct address_t *insert(struct address_t *node, struct address_t *head)
{
    ...
    if (strcmp(node->alias, head->alias) < 0)
    {
        if (DEBUG)
            ("recur left\n");
        // this will be child of head therefore depth of this node needs incrementing
        head->leftChild = insert(node, head->leftChild);
        head->height = 1 + node_max_height(head->leftChild, head->rightChild);
    }
    else
    {
        if (DEBUG)
...
struct address_t *deleteNode(struct address_t *root, char *alias)
{
    ...
    if (root == NULL)
        return root;
    // no need to recalc subtree recursively in the recursive case but i has
    // to be done for current node after recursing
    ...
    else
    {
        ...
        // the only child will have height of zero but will be a depth of parent-1
        // it will also inherent parent's parent
        // if (root-)
...
struct address_t *search_in_list(void *key, struct address_t *head, int (*compare)(void *, struct address_t *))
{
    if (head == NULL)
        return NULL;
...
struct address_t *prompt_address_add(struct address_t *head)
{
    ...
    do
    {
        ...
        if (!valid)
            fprintf(stderr, "Error: enter alias with length 10 or less.\n");
    } while (!valid);
...
int validate_addr(int *octet, int matched, struct address_t *head)
{
    ...
    // TODO consider replacing searchInOrder with foldWhile which is more generic
    // possibly build a linked list with the matches?
    // something like foldWhile(octet,head,&compare_addr,insert_ll, Null head )
...
struct address_t *update_address(struct address_t *head)
{
    ...
    if (validate_addr(tmp_addr, 4, head))
    {
         ...
         *
         * because alias and addr and interactively validated, I am not sure I can
         * get away from those checks.
         * but steps 3 and 4 can probably be combined
         */
...
int display_alias_for_location(struct address_t *head)
{
    ...
    acc.p = foldWhile(half_addr, node, compare_half, print_alias, (void *)0);
    n_matched = acc.i;
    if (DEBUG)
        printf("n_matched=%i\n", n_matched);
    if (!n_matched)
...
struct address_t *delete_user_address(struct address_t *head)
{
    ...
    if (input[0] == 'y')
    {
        ...
        {
            // TODO
            perror("Failed to delete node: reason unknown OR EMPTY TREE\n");
            return head;
        }
        else
            return ret;
...
int main(void)
{
    ...
    //    reduce(treeify[node,node ->  node],map(nodeify[char* -> node], peoples))
    return 0;
}