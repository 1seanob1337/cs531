...
int display_alias_for_location(struct struct_address_t** head,
                               struct struct_address_t** curr)
{
    ...
    {
        n_matched+=1;
        printf("%s",node_string(node));
        node=node->next;
    }
    if(!n_matched) fprintf(stderr, "error location does not exst within the list\n");
    return 0;
}
                                            ...
                                            )
{
    ...
    if(!valid) fprintf(stderr,"Error: enter alias with length 10 or less.\n");
    }while(!valid);
